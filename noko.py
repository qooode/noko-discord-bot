...
async def connect_trakt(ctx):
    ...
    embed = discord.Embed(
        ...
        description=f"1. Click [here]({auth_url}) to authorize with Trakt.tv\n"
                   ...
                   f"3. Use `{config.COMMAND_PREFIX}authorize <code>` with your code",
        color=0x00ff00
    )
    await ctx.send(embed=embed)

@bot.command(name='authorize')
async def authorize_trakt(ctx, code: str):
    """Authorize with Trakt.tv using the provided code."""
    try:
        # Exchange code for token
        token_data = trakt_api.exchange_code_for_token(code)
        if not token_data:
            await ctx.send("❌ Invalid authorization code. Please try again.")
            return
        
        # Get user profile
        user_profile = trakt_api.get_user_profile(token_data['access_token'])
        if not user_profile:
            await ctx.send("❌ Failed to get user profile. Please try again.")
            return
        ...
        if success:
            ...
            embed.add_field(
                name="Privacy",
                value=f"Your profile is **private** by default. Use `{config.COMMAND_PREFIX}public` to make it visible to others.",
                inline=False
            )
            await ctx.send(embed=embed)
        else:
            await ctx.send("❌ Failed to save your account data. Please try again.")
            
    except Exception as e:
        await ctx.send(f"❌ Error during authorization: {str(e)}")

@bot.command(name='public')
async def set_public(ctx):
    """Make your profile public so others can see your activity."""
    user = db.get_user(str(ctx.author.id))
    if not user:
        await ctx.send(f"❌ You need to connect your Trakt.tv account first. Use `{config.COMMAND_PREFIX}connect`")
        return
    
    if db.set_user_privacy(str(ctx.author.id), True):
        await ctx.send("✅ Your profile is now **public**! Others can see your watching activity.")
    else:
        await ctx.send("❌ Failed to update your privacy settings.")

@bot.command(name='private')
async def set_private(ctx):
    ...
    if db.set_user_privacy(str(ctx.author.id), False):
        await ctx.send("✅ Your profile is now **private**.")
    else:
        await ctx.send("❌ Failed to update your privacy settings.")

@bot.command(name='search')
async def search_content(ctx, *, query: str):
...
async def get_info(ctx, *, query: str):
    ...
    if not detailed_info:
        await ctx.send("❌ Failed to get detailed information.")
        return
...
async def mark_watched(ctx, *, query: str):
    ...
    if success:
        ...
        await ctx.send("❌ Failed to mark as watched. Please try again.")

@bot.command(name='unwatch')
async def unmark_watched(ctx, *, query: str):
    ...
    if success:
        ...
        await ctx.send("❌ Failed to remove from watched. Please try again.")

@bot.command(name='watchlist')
async def add_to_watchlist(ctx, *, query: str):
    ...
    if success:
        ...
        await ctx.send("❌ Failed to add to watchlist. Please try again.")

@bot.command(name='watching')
async def get_watching(ctx, user_mention: Optional[str] = None):
    """See what you or another user is currently watching."""
    if user_mention:
        # Extract user ID from mention
        mentioned_user_id = re.sub(r'[<@!>]', '', user_mention)
        target_user = db.get_user(mentioned_user_id)
        
        if not target_user:
            await ctx.send("❌ That user hasn't connected their Trakt.tv account.")
            return
        
        if not target_user.get('is_public', False):
            await ctx.send("❌ That user's profile is private.")
            return
    ...
    if not watching:
        name = f"**{username}**" if user_mention else "You"
        await ctx.send(f"📺 {name} are not watching anything right now.")
        return
...
async def get_last_watched(ctx, user_mention: Optional[str] = None, count: int = 5):
    ...
    if not history:
        name = f"**{username}**" if user_mention else "You"
        await ctx.send(f"📺 {name} haven't watched anything recently.")
        return
...
async def add_reminder(ctx, *, show_name: str):
    ...
    if not results:
        await ctx.send(f"❌ No shows found for '{show_name}'")
        return
    ...
    # Add reminder
    success = db.add_reminder(str(ctx.author.id), show_id, show['title'])
    ...
    if success:
        ...
        await ctx.send("❌ Failed to add reminder. You might already have one for this show.")

@bot.command(name='unremind')
async def remove_reminder(ctx, *, show_name: str):
    ...
    # Remove reminder
    success = db.remove_reminder(str(ctx.author.id), show_id)
    ...
    if success:
        ...
        await ctx.send("❌ You don't have a reminder set for this show.")
...
async def list_reminders(ctx):
    ...
    reminders = db.get_user_reminders(str(ctx.author.id))
